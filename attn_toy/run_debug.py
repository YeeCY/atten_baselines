...
from attn_toy.env.noisy_fourrooms import FourroomsDynamicNoise3, FourroomsDynamicNoise2, FourroomsDynamicNoise, \
    ImageInputWarpper, FourroomsRandomNoise, FourroomsOptimalNoise
...
from attn_toy.run_gridworld_generalize import make_gridworld
import os
import numpy as np
...
def train(env, load_path, num_timesteps, policy, nminibatches=4, n_steps=128, replay_buffer=None):
    """
    ...
    for step in range(num_timesteps):
        ...
        obses.append(obs)
        hybrid_agent.observe(action, reward, obs[np.newaxis, ...], done, train=False)
...
def main():
    ...
    env = make_gridworld(noise_type=5, seed=args.seed, optimal_action=optimal_action)()

    train(env, args.load_path, num_timesteps=args.num_timesteps, policy=args.policy, replay_buffer=replay_buffer)
